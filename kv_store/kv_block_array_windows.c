...
{
    block_array->fd = _open(file_name, O_RDWR | O_BINARY | (create ? (O_CREAT | O_TRUNC) : 0), _S_IREAD | _S_IWRITE);
    if (block_array->fd == -1)
    {
        return errno;
    }
...
int kv_block_array__read_block(const struct kv_block_array *block_array, uint32_t block, uint8_t *block_data)
{
    ...
    if (lseek(block_array->fd, (long)(block * block_array->raw_block_bytes), SEEK_SET) == -1)
    {
        return errno;
    }

    int ret = read(block_array->fd, block_data, (unsigned)block_array->raw_block_bytes);
    if (ret == -1)
    {
        return errno;
    }
...
int kv_block_array__write_block(struct kv_block_array *block_array, uint32_t destination_block, const uint8_t *block_data)
{
    ...
    if (lseek(block_array->fd, (long)(destination_block * block_array->raw_block_bytes), SEEK_SET) == -1)
    {
        return errno;
    }

    int ret = write(block_array->fd, block_data, (unsigned)block_array->raw_block_bytes);
    if (ret == -1)
    {
        return errno;
    }
