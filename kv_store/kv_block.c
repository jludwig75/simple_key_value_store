...
kv_block_header__init_empty(struct kv_block_header *header)
{
	header->signature = 0;
	header->key_id = 0;
	header->sequence = 0;
...
kv_block_header__init(struct kv_block_header *header, uint64_t key_id, uint32_t data_bytes, uint64_t sequence)
{
...
kv_block__init_empty(struct kv_block *kv_block)
{
	kv_block_header__init_empty(&kv_block->header);
}

kv_block__init(struct kv_block *kv_block, uint64_t key_id, uint32_t data_bytes, const char *value_data, uint64_t sequence)
{
