...
static size_t block_to_byte(const uint32_t block)
{
...
static size_t block_to_bit(uint32_t block)
{
	assert(block < _number_of_blocks);
...
static bool kv_blocmk_allocator__is_set(const struct kv_block_allocator *block_allocator, uint32_t block)
{
	return ((1 << block_to_bit(block)) & block_allocator->bits[block_to_byte(block)]) != 0;
}

static void kv_blocmk_allocator__set(struct kv_block_allocator *block_allocator, uint32_t block)
{
	block_allocator->bits[block_to_byte(block)] |= (1 << block_to_bit(block));
}
...
uint32_t kv_block_allocator__find_next_free_block(const struct kv_block_allocator *block_allocator, uint32_t starting_block)
{
	...
	{
		if (!kv_blocmk_allocator__is_set(block_allocator, b))
		{
			return b;
		}
	}
